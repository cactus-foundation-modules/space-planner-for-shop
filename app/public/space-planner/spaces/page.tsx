      ...
      {withPlans.map(({ entry, plans }) => (
        <section key={entry.room.id} style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md, 10px)', padding: '1rem', display: 'grid', gap: '0.6rem' }}>
          ...
          <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'grid', gap: '0.35rem' }}>
            {plans.map((plan) => (
              <li key={plan.id} style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
                <span>{plan.name}</span>
            ...
            {plans.length === 0 && (
              <li style={{ color: 'var(--color-text-muted)', fontSize: 'var(--text-sm, 0.875rem)' }}>No layouts in this one yet.</li>
            )}
          </ul>
        </section>
      ))}
